...
class Model(object):
    ...
    def set_params(self, x):
        ...
        for v in self.get_trainable_variables():
            val = v.get_value()
            s = val.size
            v.set_value(x[pos:pos+s].astype(theano.config.floatX).reshape(val.shape))
            pos += s
...
class TrainingLog(object):
    ...
    def __init__(self, filename, attrs, appendFlag):
        if filename is None:
            ...
            if not appendFlag:
                ...
                try: 
                    self.log_table = self.f.root.log
                except:
                    self.log_table = None
...
class Standardizer(Model):
    def __init__(self, dim, eps=1e-6, init_count=0, init_mean=0., init_meansq=1.):
        ...
        with variable_scope(type(self).__name__) as self.__varscope:
            ...
            self._mean_1_D = get_variable('mean_1_D', np.full((1, self._dim), init_mean,dtype=theano.config.floatX), broadcastable=(True,False), trainable=False)
            self._meansq_1_D = get_variable('meansq_1_D', np.full((1, self._dim), init_meansq,dtype=theano.config.floatX), broadcastable=(True,False), trainable=False)
...
def test_standardizer():
    ...
    x_N_D = np.random.randn(200, D,dtype=theano.config.floatX)
    s.update(x_N_D)

    x2_N_D = np.random.randn(300, D,dtype=theano.config.floatX)
    s.update(x2_N_D)
