...
NGStepInfo = namedtuple('NGStepInfo', 'obj0, kl0, obj1, kl1, gnorm, bt, sum_grad')
def make_ngstep_func(model, compute_obj_kl, compute_obj_kl_with_grad, compute_kl_hvp):
    ...
    def wrapper(feed, max_kl, damping, subsample_hvp_frac=.1, grad_stop_tol=1e-6, max_cg_iter=10, enable_bt=True):
        ...
        params1, num_bt_steps = ngstep(
            x0=params0,
            obj0= -obj0, # AS: This objective needs to be minimized - hence the negative sign
            objgrad0= -objgrad0, # TODO: Check if there is a minimization or maximization going on: minimization acc to theory
