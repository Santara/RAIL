...
class Policy(nn.Model):
    def __init__(self, obsfeat_space, action_space, num_actiondist_params, enable_obsnorm, varscope_name, useCVaR=False, 
        CVaR_loss_weightage = 1. # This is just for the ablation study - not theoretically compliant
        ):
        self.obsfeat_space, self.action_space, self._num_actiondist_params, self._useCVaR, self._CVaR_loss_weightage = obsfeat_space, action_space, num_actiondist_params, useCVaR, CVaR_loss_weightage
        ...
        obj_AL = (impweight_B*advantage_B).mean()

        # Composing the CVaR term
        CVaR_weights_placeholder = tensor.vector(name="CVaR_weights")
        if self._useCVaR:
            obj_CVaR = (logprobs_B*CVaR_weights_placeholder).mean() # DANGER: This should have been mean of the sum over traj lengths
            # objgrad_P = thutil.flatgrad(obj_AL+obj_CVaR, param_vars)#FIXME: <<<<<<<<<<<<<<<<<<<<<<<<< Add the new gradients here
            obj = obj_AL - self._CVaR_loss_weightage*obj_CVaR #Negative sign for minimizing the CVaR loss
        else:
            obj = obj_AL
        
        objgrad_P = thutil.flatgrad(obj, param_vars) #This apparently solves the convergence problem
        # Gradients for debugging
        # diag_grad_cost_AL = thutil.flatgrad(obj, param_vars)


        # KL divergence from old policy 
        # Nothing of interest to CVaR
        kl_B = self._make_actiondist_kl_ops(proposal_actiondist_B_Pa, actiondist_B_Pa)
        kl = kl_B.mean()
        #TODO: make two separate definitions with and without CVaR
        if self._useCVaR:
            compute_obj_kl = thutil.function([obsfeat_B_Df, input_actions_B_Da, proposal_actiondist_B_Pa, advantage_B, CVaR_weights_placeholder], [obj, kl], on_unused_input='ignore') ## DANGER:should we return obj or obj+obj_CVaR?
            compute_obj_kl_with_grad = thutil.function([obsfeat_B_Df, input_actions_B_Da, proposal_actiondist_B_Pa, advantage_B, CVaR_weights_placeholder], [obj, kl, objgrad_P], on_unused_input='ignore')
        else:
            compute_obj_kl = thutil.function([obsfeat_B_Df, input_actions_B_Da, proposal_actiondist_B_Pa, advantage_B], [obj, kl], on_unused_input='ignore') ## DANGER:should we return obj or obj+obj_CVaR?
            compute_obj_kl_with_grad = thutil.function([obsfeat_B_Df, input_actions_B_Da, proposal_actiondist_B_Pa, advantage_B], [obj, kl, objgrad_P], on_unused_input='ignore')
        ...
        if self._useCVaR:
            compute_hvp = lambda _obsfeat_B_Df, _input_actions_B_Da, _proposal_actiondist_B_Pa, _advantage_B, _CVaR_weights, _v_P: hvp(_obsfeat_B_Df, _proposal_actiondist_B_Pa, _v_P)
        else:
        ...
        self.gradients_for_debugging = thutil.function([obsfeat_B_Df, input_actions_B_Da, proposal_actiondist_B_Pa, advantage_B, CVaR_weights_placeholder],objgrad_P, on_unused_input='ignore')
...
class GaussianPolicy(Policy):
    def __init__(self, cfg, obsfeat_space, action_space, varscope_name, useCVaR=False, CVaR_loss_weightage = 1.):
        ...
        Policy.__init__(
            ...
            useCVaR=useCVaR,
            CVaR_loss_weightage=CVaR_loss_weightage)
    ...
    def _make_actiondist_ops(self, obsfeat_B_Df):
        ...
        with nn.variable_scope('out'):
            mean_layer = nn.AffineLayer(net.output, net.output_shape, (self.action_space.dim,), initializer=np.zeros((net.output_shape[0], self.action_space.dim),dtype=theano.config.floatX))
        ...
        logstdevs_1_Da = nn.get_variable('logstdevs_1_Da', np.full((1, self.action_space.dim), self.cfg.init_logstdev,dtype=theano.config.floatX), broadcastable=(True,False))
...
def TRPO(max_kl, damping, subsample_hvp_frac=.1, grad_stop_tol=1e-6, useCVaR=False):
    ...
    def trpo_step_with_CVaR(policy, params0_P, obsfeat, a, adist, adv, 
        CVaR_weights                                
        ):
        feed = (obsfeat, a, adist, util.standardized(adv), CVaR_weights) 
        

        stepinfo = policy._ngstep(feed, max_kl=max_kl, damping=damping, subsample_hvp_frac=subsample_hvp_frac, grad_stop_tol=grad_stop_tol)
        return [
            ('dl', stepinfo.obj1 - stepinfo.obj0, float), # improvement of penalized objective
            ('pgrad', stepinfo.sum_grad, float),
            # ('kl', stepinfo.kl1, float), # kl cost of solution
            # ('gnorm', stepinfo.gnorm, float), # gradient norm
            # ('bt', stepinfo.bt, int), # number of backtracking steps
        ]
...
class LinearValueFunc(object):
    ...
    def _feat(self, obs_B_Do, t_B):
        ...
        return np.concatenate([
                ...
                t_B[:,None]/100.,
                (t_B[:,None]/100.)**2,
                np.ones((B,1),dtype=theano.config.floatX)
            ], axis=1)
    ...
    def fit(self, obs_B_Do, t_B, y_B):
        ...
        self.w_Df = scipy.linalg.solve(
            feat_B_Df.T.dot(feat_B_Df) + self.l2reg*np.eye(feat_B_Df.shape[1],dtype=theano.config.floatX),
            feat_B_Df.T.dot(y_B),
            sym_pos=True)
...
class ConstantValueFunc(object):
    def __init__(self, max_timesteps):
        ...
        self.v_T = np.zeros(max_timesteps,dtype=theano.config.floatX)
    ...
    def fit(self, obs_B_Do, t_B, y_B):
        ...
        sum_T = np.zeros(self.max_timesteps,dtype=theano.config.floatX)
        ...
        counts_T = np.zeros(self.max_timesteps,dtype=theano.config.floatX)
